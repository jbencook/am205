...
\subsection{SVD}
As with other matrix factorization techniques, SVD works by decomposing a matrix, $\mathbf{A}$ into separate matrices whose product is $\mathbf{A}$:
$$\textrm{SVD}(\mathbf{A}) = \mathbf{U}\mathbf{S}\mathbf{V}^{\textrm{T}}$$
where $\mathbf{U}$ is blank, $\mathbf{S}$ is blank, and $\mathbf{V}$ is blank \cite{golub1970}.

Although we did try to implement SVD from scratch following the alogrithm laid out in \cite{recipes2007}, it quickly became clear that implementing SVD from scratch was beyond the scope of this project.
